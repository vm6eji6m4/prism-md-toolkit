...
PRISM 把這套閱讀順序打包給 AI：專案地圖、星等排序、逐檔摘要、可稽核的 manifest。12-cell A/B 實測中，它在 45 檔 repo 上把答對率從 6.5 拉到 8/8 且出處全對——**地圖省的是錯誤，不是 token**。（誠實的例外：本機單發小模型還需要 harness 端供檔配合——見實測報告。）
...
整個專案 → 星等分級導航包＋manifest（每檔 sha256／token 估計／星等／摘要）。串流寫出——上萬檔案的樹也不會爆記憶體。地圖到底有沒有用？誠實的 12-cell A/B 實測（3 模型 × 2 語料、陷阱題負對照）：[docs/MAP_VALUE_EVAL.md](docs/MAP_VALUE_EVAL.md)。

### 🎓 PRISM Skill Pack — `--skill` 模式

任何資料夾 → [Agent Skills](https://agentskills.io) 相容知識包：`SKILL.md`（閱讀守則＋Reading Ledger 規格＋星等地圖）＋小檔優先編號的 `references/`＋sha256 manifest。守則是引導不是強制——所以配 `verify_ledger()` 把台帳聲稱對實際供檔記錄機器對賬（複測實際抓到小模型聲稱讀了 5 個從未給它的檔）。
...
# 打包成 Agent Skills 相容知識包（守則＋Reading Ledger＋地圖）
python prism/prism_pack.py <來源資料夾> --skill -o <輸出資料夾> --template qa
...
- **打包前密鑰掃描**（`--skill` 模式）——命中憑證樣式（API key／token／私鑰）的檔案不進包，並記錄於 manifest

以上閘門全數有**負對照測試**（證明它真的會拒絕）：
`python -m unittest discover tests`——20 條零依賴測試，Python 3.11/3.14 雙版本全綠。
...
執行代理起草初版 → 審核代理親驗、修 bug、安全加固 → 上架前經三家獨立 AI 評審——精簡稽核摘要見 [docs/AUDIT_LOG.md](docs/AUDIT_LOG.md)。產品負責：[vm6eji6m4](https://github.com/vm6eji6m4)（國裕）。
