...
- **星等＝啟發式，且判準綁程式碼倉**：檔名/路徑關鍵字判定（找 CLI/README/config/核心/test），重新命名會改變星等；非內容理解。**在非程式碼語料（純文件庫/文集/論文集）上此判準無區辨力→整包星等全攤平同級**（實測一批 ~1,400 檔、~7.5M token 的內容型語料被打成同一星，`auto_summary` 對 .md 也只抽標題＋前幾個 header）。此時混合包退化成全嵌、失去「重要者全文/樣板僅索引」的好處，需一支領域專屬 priority＋summary 函式重打星（見 Roadmap「內容型語料 profile」）。
