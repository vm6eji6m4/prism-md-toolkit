...
| hard_doc.pdf | 12.3MB | **直式排版中文手冊** | 1 | ❌ 完全失敗：直排字被逐字打散（例：「直排文字」讀成「直／排／文／字」），無章節結構可切 |
...
被讀成垂直散字，句子完全解體。
**影響**：**傳統中文文書（古籍、手冊、公文類）多為直排**——這是東亞文件場景的主要缺口。
**修法方向**：PyMuPDF block bbox 可判向（高瘦 block 群＝直排）→ 偵測後改用直排讀取順序；
或該頁降級走 OCR。屬「值得做」級；外審共識已列入 README Roadmap（直排 CJK 支援）。
