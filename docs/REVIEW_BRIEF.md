...
| W11 | 論文管線：**直排 CJK 版面完全失敗**（12MB 直式中文手冊只出 1 片散字，F2）——傳統中文文書場景的硬缺口 | bbox 判向改讀取順序，或降級 OCR |
...
8. **W11 直排 CJK** 是傳統中文文書場景的硬缺口：bbox 判向 vs 整頁 OCR 降級 vs 宣告不支援並文件註明，哪條路 ROI 最好？有無現成庫（如 PyMuPDF 4.x 的 text direction API）可借力？
